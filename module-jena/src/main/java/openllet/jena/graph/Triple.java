 ...
 */
public class Triple extends org.apache.jena.graph.Triple implements Serializable
{
    ...
    public String toString(PrefixMapping pm) {
        return stringForNode(pm, subj) + " " + stringForNode(pm, pred) + " " + stringForNode(pm, obj);
    }

    // Display string for node.
    private String stringForNode(PrefixMapping pm, Node node) {
        if ( node.isURI() ) {
            if ( pm != null ) {
                String x = pm.shortForm(node.getURI());
                if ( x != null )
                    return x;
            }
            return "<"+node.getURI()+">";
    ...
    protected Object writeReplace() throws ObjectStreamException {
        Function<org.apache.jena.graph.Triple, Object> function =  Serializer.getTripleSerializer() ;
        if ( function == null )
